...
If indexing differs in production (profile label drift is a known risk per AIEX-509), pause the suite and resolve the canonical mapping first — the entire audit is meaningless against a mis-identified Pattern 2.
...
**Purpose:** Test the Clifford pure-grade hypothesis. AIEX-059 established that all 48 bilateral pairs produce pure grade-2 bivectors in Cl(7,0). If Pattern 2 is the cross-framework anchor, it may have a distinguished Clifford representation — e.g., a pure bivector in a lower-dimensional Clifford algebra Cl(4,0) or Cl(5,0), while other patterns require Cl(7,0) or mixed-grade elements.
...
**Purpose:** Phase 69 (April 13, 2026; AIEX-404) established two canonical values via CAILculator v1.x ZDTP:
...
def test_phase_69_zdtp_regression():
    # Load Phase 69 source data from AIEX-404 JSON archive
...
**Note on Phase 69 inputs:** AIEX-404 records that "Five Phase 69 CAILculator JSON files [were] generated with raw output values for open science record." These are the authoritative inputs. If the JSON files are not directly accessible, reconstruct from the Phase 69 handoff document or KSJ connections to AIEX-404.
...
- **Large drift ($>10^{-8}$):** indicates ZDTP 2.0's bilateral-grounding (§ README "ZDTP 2.0") changes the protocol output meaningfully. Not necessarily wrong — the new protocol is more formally grounded — but all prior Phase 69 citations need re-verification, and AIEX-404's headline value must be updated or contextualized.
...
Per the v2.0 non-triviality discipline (AIEX-441, AIEX-482 lesson):
...
- **Pattern 2 contribution** must show per-pattern variation. If all six patterns return bit-identical convergence scores, the test is probing in a regime where K_Z_realToSed (AIEX-477) still collapses the signal — adjust inputs (non-scalar, asymmetric, aligned with the Option A embedding) until genuine differentiation appears.
...
## 8. Planning Notes for Gemini CLI
...
- Phase 69 inputs (AIEX-404) must be loaded from the authoritative JSON archive, not reconstructed from KSJ text. If the archive files are not in the repository, locate them before running B.1 — reconstructing inputs would invalidate the regression test.
...
- **AIEX-404** — Phase 69 headline: SG prime ZDTP convergence 0.9866936838248778 vs Riemann zero ZDTP ceiling 0.9577023861134006 (April 13, 2026)
- **AIEX-174** — Universal bilateral annihilation across all 50 zeros and 6 gateways
- **AIEX-358** — Canonical Six as single Weyl orbit under E8
- **AIEX-361** — Dual-algebraic bilateral ZD validity across Cayley-Dickson and Clifford
- **AIEX-059** — All 48 bilateral pairs produce pure grade-2 bivectors in Cl(7,0)
- **AIEX-477** — K_Z_realToSed exact theorem (scalar input dispatch collapse)
- **AIEX-509** — Model-level uncertainty about Canonical Six membership; profile label drift risk
